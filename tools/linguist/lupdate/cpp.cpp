                               ...
                               QSet<QString> &inclusions)
{
    ...
    if (inclusions.contains(cleanFile)) {
        yyMsg() << qPrintable(LU::tr("circular inclusion of %1").arg(cleanFile)) << std::endl;
        return;
    }
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        yyMsg()
            << qPrintable(LU::tr("Cannot open %1: %2").arg(cleanFile)
            .arg(f.errorString())) << std::endl;
        return;
    }
                ...
                if (!pendingContext.isEmpty() && !prefix.startsWith(strColons)) {
                    ...
                    if (!fullyQualify(namespaces, pendingContext, true, &functionContext, &unresolved)) {
                        ...
                        yyMsg() << qPrintable(LU::tr("Qualifying with unknown namespace/class %1::%2")
                                .arg(stringifyNamespace(functionContext)).arg(unresolved.first())) << std::endl;
                    }
                    pendingContext.clear();
                }
                if (prefix.isEmpty()) {
...
void loadCPP(Translator &translator, const QStringList &filenames, ConversionData &cd)
{
    ...
    foreach (const QString &filename, filenames) {
        ...
        if (!file.open(QIODevice::ReadOnly)) {
            cd.appendError(LU::tr("Cannot open %1: %2")
