...
QString Translator::guessLanguageCodeFromFileName(const QString &filenameOrig)
{
    // remove directory, and file extension
    const QString filename = QFileInfo(filenameOrig).fileName();
    ...
    // first break apart based only on ".": this is to try to match language_COUNTRY in the filename first.
    //  Then try separating based on "_" or "." to find more possibilities.
    static QRegExp re(QLatin1String("[\\._]"));
    QStringList toCheck = str.split(QLatin1String("."), QString::SkipEmptyParts) +
                          str.split(re, QString::SkipEmptyParts);
    foreach(const QString &part, toCheck) {
        QLocale locale(part);
        //qDebug() << "LANGUAGE FROM " << part << "LANG CODE: " << locale.language();
