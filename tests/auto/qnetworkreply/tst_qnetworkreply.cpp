...
public:
    ...
    MiniHttpServer(const QByteArray &data, bool ssl = false)
        : socket(0), dataToTransmit(data), doClose(true), doSsl(ssl)
    {
        listen(QHostAddress(QHostAddress::LocalHost), 0);
...
private:
    ...
    {
        //qDebug() << "connectSocketSignals" << socket;
...
private slots:
    ...
    {
        qDebug() << "slotSslErrors" << socket->errorString() << errors;
    }
#endif
    void slotError(QAbstractSocket::SocketError err)
    {
        qDebug() << "slotError" << err << socket->errorString();
    }

    void sendData()
    {
        QByteArray httpRequest = socket->readAll();
        //qDebug() << httpRequest;
        receivedData += httpRequest;
