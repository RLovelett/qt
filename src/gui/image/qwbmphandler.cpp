...
#include "private/qwbmphandler_p.h"
...
bool WBMPReader::canRead(QIODevice *device)
{
    if (device) {
        ...
        if (device->isSequential()) {
            qWarning("QWbmpHandler::canRead() called with a sequential device");
            return false;
        }
        ...
        if (readWBMPHeader(device, &hdr)) {
            if ((hdr.bType == 0) && (hdr.bFormat == 0)) {
                qint64 imageSize = hdr.iHeight * ((hdr.iWidth + 7) / 8);
