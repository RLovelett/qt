...
bool QTreeViewPrivate::expandOrCollapseItemAtPos(const QPoint &pos, const bool recurse)
{
    Q_Q(QTreeView);
...
void QTreeViewPrivate::layout(int itemNum, bool expandBranch, bool afterIsUninitialized)
{
    ...
    // all visible children of 'itemNum' in the unvisitedChildren stack.
    unvisitedChildren.clear();
    int level = (itemNum >= 0) ? viewItems.at(itemNum).level : -1;
    bool moreVisibleRows = false;
    for (int row=numChildren-1; row>=0; row--) {
        ...
        if (expandBranch || !isRowHidden(childIndex)) {
            unvisitedChildren.push (LayoutData (childIndex, itemNum, level+1, moreVisibleRows));
            moreVisibleRows = true;
        }
    }
        ...
        if (expanded) {
            ...
            for (int row=numChildren-1; row>=0; row--) {
                ...
                if (isRowHidden(childIndex))
                    ...
                    unvisitedChildren.push(LayoutData(childIndex, parentItem, childDepth, moreVisibleRows));
                    moreVisibleRows = true;
                }
            }
