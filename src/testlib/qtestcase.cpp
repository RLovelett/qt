...
/*! \fn bool QTest::qCompare(int const &t1, int const &t2, int const &t3, const char *actual, const char *expected, const char *fuzz, const char *file, int line)
\internal
 */
Q_TESTLIB_EXPORT bool QTest::qCompare(int const &t1, int const &t2, int const &t3,
		     const char *actual, const char *expected, const char *fuzz,
		     const char *file, int line)
{
    return (qAbs(t1 - t2) <= t3)
            ? compare_helper(true, "FUZZ_COMPARE()", file, line)
            : compare_helper(false, "Compared integers are not the same (fuzzy compare)",
                             toString(t1), toString(t2), toString(t3),
			     actual, expected, fuzz, file, line);
}

/*! \fn bool QTest::qCompare(float const &t1, float const &t2, float const &t3, const char *actual, const char *expected, const char *fuzz, const char *file, int line)
\internal
 */
Q_TESTLIB_EXPORT bool QTest::qCompare(float const &t1, float const &t2, float const &t3,
		     const char *actual, const char *expected, const char *fuzz,
		     const char *file, int line)
{
    return (qAbs(t1 - t2) <= t3)
...
/*! \fn bool QTest::qCompare(double const &t1, double const &t2, double const &t3, const char *actual, const char *expected, const char *fuzz, const char *file, int line)
\internal
 */
Q_TESTLIB_EXPORT bool QTest::qCompare(double const &t1, double const &t2, double const &t3,
		     const char *actual, const char *expected, const char *fuzz,
		     const char *file, int line)
{
    return (qAbs(t1 - t2) <= t3)
