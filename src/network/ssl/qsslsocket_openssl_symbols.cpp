...
DEFINEFUNC2(void, SSL_CTX_set_client_cert_cb, SSL_CTX *ctx, ctx, int (*a)(SSL *, X509 **, EVP_PKEY **), a, return, return)
DEFINEFUNC5(int, SSL_CTX_get_ex_new_index, long argl, argl, void *argp, argp, CRYPTO_EX_new *new_func, new_func, CRYPTO_EX_dup *dup_func, dup_func, CRYPTO_EX_free *free_func, free_func, return 0, return)
DEFINEFUNC3(int, SSL_CTX_set_ex_data, SSL_CTX *ctx, ctx, int idx, idx, void *arg, arg, return -1, return)
DEFINEFUNC2(void *, SSL_CTX_get_ex_data, SSL_CTX *ctx, ctx, int idx, idx, return 0, return)
#if OPENSSL_VERSION_NUMBER >= 0x10000000L
...
bool q_resolveOpenSslSymbols()
{
    ...
    RESOLVEFUNC(SSL_CTX_get_ex_new_index)
    RESOLVEFUNC(SSL_CTX_set_ex_data)
    RESOLVEFUNC(SSL_CTX_get_ex_data)
